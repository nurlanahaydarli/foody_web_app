...
import { useDeleteRestuarantMutation, useEditRestuarantMutation, useGetRestuarantsQuery } from "../../../redux/global/RestaurantApi";
...
function AdminRestaurant() {
  ...
  const [DeliveryPriceValue,setDeliveryPriceValue ] = useState<number|string>('');
  const [DeliveryPriceStroge, setDeliveryPriceStroge] = useState('');

  const [DeliveryMinValue, setDeliveryMinValue] = useState<number|string>('');
  ...
  const {data, isLoading, isError} = useGetRestuarantsQuery({},{
  });
  

  const [deleteRestuarant] = useDeleteRestuarantMutation();
  const [updateRestuarantApi] = useEditRestuarantMutation();
...
function getCategorysById(e:any) {
  ...
  async function updateRestuarant() {
    const addressRegex = /^[a-zA-Z0-9\s,'-]*$/;
    ...
    await updateRestuarantApi(updatedRestaurant);
    ...
    try {
      ...
      dispatch({
        ...
        payload: state.restaurantData.map((restaurant) =>
          restaurant.id === editID ? updatedRestaurant : restaurant
        ),
      });
      toast({
  ...
  {state.restaurantData
    ...
    .map((restaurant: Restaurant, index: number) => {
        ...
        <div key={index} className="bg-gray-800 shadow-black rounded-lg p-4 flex flex-col items-center mb-4">
          <div className="overflow-hidden relative flex shadow-black items-center space-x-4 bg-white rounded-lg p-4 w-full h-36">
            ...
            <div className="flex flex-col justify-center">
              <h2 className="text-xl font-semibold text-black truncate max-w-full">{shortText(restaurant.name, 10)}</h2>
      ...
      <Form
        ...
        onAction={editImg ? updateRestuarant : addRestaurant}
